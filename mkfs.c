...
static int write_inode_store(int fd, struct superblock *sb)
{
    ...
    uint32_t first_data_block = 1 + le32toh(sb->info.nr_istore_blocks) +
...
static int write_ifree_blocks(int fd, struct superblock *sb)
{
    ...
    /* The initial ifree block hold the first inode number as in-use, inode 0 and inode 1 is set as 0 */
    ...
    uint32_t i;
    for (i = 1; i < sb->info.nr_ifree_blocks; i++) {
...
static int write_bfree_blocks(int fd, struct superblock *sb)
{
    ...
    uint32_t nr_blocks_used = le32toh(sb->info.nr_istore_blocks) +
                              le32toh(sb->info.nr_ifree_blocks) +
